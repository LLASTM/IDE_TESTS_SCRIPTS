...
When('user selects menu {string}', { timeout: 60 * 1000 }, async(menu:string) =>{
    ...
    //new Promise( resolve => setTimeout(resolve, 1000) ); // Added by [LLA]
    subMenus.shift();
...
When('user builds all projects for {string}', async function (this: CubeWorld, products:string) {
    ...
    for(let index=1;index<currentList.length;index++)
    {
        ...
        if (deviceStatus === 'Active')
        {
            ...
            userCreatesrojectForDevice(projectName, deviceName);
        }       
    }
});
/**
...
async function userCreatesrojectForDevice(projectName:string, deviceName:string)
{
    IDEtrace('DEBUG','Entering userCreatesrojectForDevice');

    // let locatorText=`button:has-text(${productName})`;
    // await page.locator(locatorText).first().click();
