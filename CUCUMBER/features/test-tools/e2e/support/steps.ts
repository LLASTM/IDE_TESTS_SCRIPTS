...
async function dragAndDropSWComposer() {

    const delay=2;
    await page.pause();

    console.log('Entering dragAndDropSWComposer');
    
    const Yoffset=100;
    const Xoffset=400;

    const inititialLocator = page.locator('div.search-container >> div.element-list >> div.element').first();
    if (inititialLocator) { 
        await inititialLocator.click();
    ...
    if (dragBox)
    {
        ...
        console.log('dragAndDropSWComposer : move mouse from x=' + Xorigin);
        console.log('dragAndDropSWComposer : move mouse from y=' + Yorigin);
        console.log();
        console.log('dragAndDropSWComposer : move mouse to x=' + Xtarget);
        console.log('dragAndDropSWComposer : move mouse to y=' + Ytarget);

        // await page.mouse.move(Xorigin, Yorigin,{ steps:4});
        // await new Promise( resolve => setTimeout(resolve, + 2 * 1000) );
        // await page.mouse.down({button:"left"});
        // console.log('dragAndDropSWComposer:mouse is down');
        // await new Promise( resolve => setTimeout(resolve, + 2 * 1000) );
        // await page.mouse.move(Xtarget, Ytarget,{ steps:4}) ; 
        // await new Promise( resolve => setTimeout(resolve, + 4 * 1000) );

        await inititialLocator.hover();
        await new Promise( resolve => setTimeout(resolve, + delay * 1000) );
        await page.mouse.move(Xorigin, Yorigin) ;
        await new Promise( resolve => setTimeout(resolve, + delay * 1000) );

        await page.mouse.down({button:"left"});
        await new Promise( resolve => setTimeout(resolve, + delay * 1000) );
        
        await page.locator('#swcomposer-diagram_0 svg').hover();
        await page.mouse.move(Xtarget, Ytarget) ; 
        await new Promise( resolve => setTimeout(resolve, + delay * 1000) );

        await page.mouse.up({button:"left"});
        await new Promise( resolve => setTimeout(resolve, + delay * 1000) );
        console.log('dragAndDropSWComposer : mouse is up');
  
    }
    else
    {
        console.error('dragAndDropSWComposer : dragBox is null');
    }
}
// ================================================================================
...
// ================================================================================
async function userAddsCubeNoOsBlock()
{
    console.log('Entering userAddsCubeNoOsBlock');

    await page.locator('.p-TabBar-tabIcon.codicon.codicon-telescope').first().click();
    await page.locator('input[type="search"]').click();
    await page.locator('input[type="search"]').fill('cube no os');

    await dragAndDropSWComposer();
    await saveAllFiles();
...
// ================================================================================
async function userAddsHalCodeGenBlock()
{
    console.log('Entering userAddsHalCodeGenBlock');

    await page.locator('input[type="search"]').click();
    await page.locator('input[type="search"]').fill('hal code gen: generated code');
...
// ================================================================================
async function userAddsRCCInitBlock()
{
    console.log('Entering userAddsRCCInitBlock');

    await page.locator('input[type="search"]').click();
    await page.locator('input[type="search"]').fill('hal code gen: rcc init');
...
// ================================================================================
async function userAddsHalCoreBlock()
{
    console.log('Entering userAddsHalCoreBlock');

    await page.locator('input[type="search"]').click();
    await page.locator('input[type="search"]').fill('hal: core');
...
{
    console.log('Entering userResolvesDependencies');
...
async function userGeneratesCode()
{
    ...
    console.log('Entering userGeneratesCode');
    await page.locator('#swcomposer-diagram_0 svg').click({
    button: 'right'
    });
...
// ================================================================================
async function userDisablesDMATimerIT()
{
    console.log('Entering userDisablesDMATimerIT');

    //await page.pause();

     const selectorText='div[id="swcomposer-diagram_0"].sprotty >> svg.sprotty-graph >> g >> g.node.component';
    
    const locatorList=page.locator(selectorText);
    const locatorCount=await locatorList.count();

    console.log('Locator list width is ' + locatorCount);
    ...
    for(let index=0;index<locatorCount;index++)
    {
        ...
        if (searchedNode)
        {
            ...
            if (currentText === "STM32 HAL Code Gen:System Init")
            {
                ...
                const newItem=currentItem.locator('g.comp.sprotty-comp >> g.comp-instances.instances >> g.sprotty-button.enabled.configuration >> rect'); 
                console.log('search for button, width=' + await newItem.count());
     
                if (newItem) {
                
                    console.log('Before calling hover');
                    await newItem.hover();
                    console.log('After calling hover');
                    ...
                    //     console.log('click on button done');
                    // }
                    
                    // await page.frameLocator('iframe').frameLocator('#active-frame').locator('div[role="button"]:has-text("1MX")').click();
                    // await page.frameLocator('iframe').frameLocator('#active-frame').locator('[aria-label="Enable DMA usage inside the HAL TIM"] input[type="checkbox"]').uncheck();
